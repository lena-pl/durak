...
describe "Full Game" do
  ...
  it "works, because you're worth it. " do
    ...
    test_game.discard(player_two, cards(:clubs_14))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14),
                                          ...
                                          cards(:clubs_13),
                                          cards(:diamonds_13)]
    expect(game_state.discard_pile.count).to eq 11
    expect(game_state.discard_pile).to include(cards(:clubs_14))
    expect(game_state.table.arranged.fifth[:attacking_card]).to eq cards(:diamonds_13)
    expect(game_state.table.arranged.fifth[:defending_card]).to be_nil

    test_game.discard(player_two, cards(:diamonds_13))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14),
                                          ...
                                          cards(:diamonds_9),
                                          cards(:clubs_13)]
    expect(game_state.discard_pile.count).to eq 12
    expect(game_state.discard_pile).to include(cards(:diamonds_13))

    test_game.discard(player_two, cards(:clubs_13))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14),
                                          ...
                                          cards(:clubs_9),
                                          cards(:diamonds_9)]
    expect(game_state.discard_pile.count).to eq 13
    expect(game_state.discard_pile).to include(cards(:clubs_13))
    expect(game_state.table.arranged.fourth[:attacking_card]).to eq cards(:diamonds_9)
    expect(game_state.table.arranged.fourth[:defending_card]).to be_nil

    test_game.discard(player_two, cards(:diamonds_9))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14),
                                          ...
                                          cards(:diamonds_11),
                                          cards(:clubs_9)]
    expect(game_state.discard_pile.count).to eq 14
    expect(game_state.discard_pile).to include(cards(:diamonds_9))

    test_game.discard(player_two, cards(:clubs_9))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14),
                                          ...
                                          cards(:spades_14),
                                          cards(:diamonds_11)]
    expect(game_state.discard_pile.count).to eq 15
    expect(game_state.discard_pile).to include(cards(:clubs_9))
    expect(game_state.table.arranged.third[:attacking_card]).to eq cards(:diamonds_11)
    expect(game_state.table.arranged.third[:defending_card]).to be_nil

    test_game.discard(player_two, cards(:diamonds_11))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14),
                                          cards(:spades_11),
                                          cards(:spades_14)]
    expect(game_state.discard_pile.count).to eq 16
    expect(game_state.discard_pile).to include(cards(:diamonds_11))

    test_game.discard(player_two, cards(:spades_14))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14),
                                          cards(:spades_11)]
    expect(game_state.discard_pile.count).to eq 17
    expect(game_state.discard_pile).to include(cards(:spades_14))
    expect(game_state.table.arranged.second[:attacking_card]).to eq cards(:spades_11)
    expect(game_state.table.arranged.second[:defending_card]).to be_nil

    test_game.discard(player_two, cards(:spades_11))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11),
                                          cards(:hearts_14)]
    expect(game_state.discard_pile.count).to eq 18
    expect(game_state.discard_pile).to include(cards(:spades_11))

    test_game.discard(player_two, cards(:hearts_14))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:hearts_11)]
    expect(game_state.discard_pile.count).to eq 19
    expect(game_state.discard_pile).to include(cards(:hearts_14))
    expect(game_state.table.arranged.first[:attacking_card]).to eq cards(:hearts_11)
    expect(game_state.table.arranged.first[:defending_card]).to be_nil

    test_game.discard(player_two, cards(:hearts_11))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to be_empty
    expect(game_state.discard_pile.count).to eq 20
    expect(game_state.discard_pile).to include(cards(:hearts_11))
    ...
    test_game.discard(player_one, cards(:clubs_10))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to eq [cards(:diamonds_7),
                                          cards(:clubs_8),
                                          cards(:diamonds_8)]
    expect(game_state.discard_pile.count).to eq 21
    expect(game_state.discard_pile).to include(cards(:clubs_10))
    expect(game_state.table.arranged.second[:attacking_card]).to eq cards(:diamonds_8)
    expect(game_state.table.arranged.second[:defending_card]).to be_nil

    test_game.discard(player_one, cards(:diamonds_8))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to eq [cards(:diamonds_7),
                                          cards(:clubs_8)]
    expect(game_state.discard_pile.count).to eq 22
    expect(game_state.discard_pile).to include(cards(:diamonds_8))

    test_game.discard(player_one, cards(:clubs_8))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to eq [cards(:diamonds_7)]
    expect(game_state.discard_pile.count).to eq 23
    expect(game_state.discard_pile).to include(cards(:clubs_8))

    test_game.discard(player_one, cards(:diamonds_7))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to be_empty
    expect(game_state.discard_pile.count).to eq 24
    expect(game_state.discard_pile).to include(cards(:diamonds_7))
    ...
    test_game.discard(player_two, cards(:clubs_11))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:diamonds_10),
                                          cards(:diamonds_14),
                                          cards(:hearts_10)]
    expect(game_state.discard_pile.count).to eq 25
    expect(game_state.discard_pile).to include(cards(:clubs_11))

    test_game.discard(player_two, cards(:hearts_10))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:diamonds_10),
                                          cards(:diamonds_14)]
    expect(game_state.discard_pile.count).to eq 26
    expect(game_state.discard_pile).to include(cards(:hearts_10))

    test_game.discard(player_two, cards(:diamonds_14))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:diamonds_10)]
    expect(game_state.discard_pile.count).to eq 27
    expect(game_state.discard_pile).to include(cards(:diamonds_14))

    test_game.discard(player_two, cards(:diamonds_10))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to be_empty
    expect(game_state.discard_pile.count).to eq 28
    expect(game_state.discard_pile).to include(cards(:diamonds_10))
    ...
    test_game.discard(player_one, cards(:clubs_6))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to eq [cards(:diamonds_6)]
    expect(game_state.discard_pile.count).to eq 29
    expect(game_state.discard_pile).to include(cards(:clubs_6))

    test_game.discard(player_one, cards(:diamonds_6))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to be_empty
    expect(game_state.discard_pile.count).to eq 30
    expect(game_state.discard_pile).to include(cards(:diamonds_6))
    ...
    test_game.discard(player_two, cards(:spades_9))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to eq [cards(:spades_8)]
    expect(game_state.discard_pile.count).to eq 31
    expect(game_state.discard_pile).to include(cards(:spades_9))

    test_game.discard(player_two, cards(:spades_8))
    game.reload
    expect(game_state.attacker).to eq player_one
    expect(game_state.table.cards).to be_empty
    expect(game_state.discard_pile.count).to eq 32
    expect(game_state.discard_pile).to include(cards(:spades_8))
    ...
    test_game.discard(player_one, cards(:hearts_13))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to eq [cards(:hearts_9)]
    expect(game_state.discard_pile.count).to eq 33
    expect(game_state.discard_pile).to include(cards(:hearts_13))

    test_game.discard(player_one, cards(:hearts_9))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to be_empty
    expect(game_state.discard_pile.count).to eq 34
    expect(game_state.discard_pile).to include(cards(:hearts_9))
    ...
    test_game.pick_up_from_table(player_one, cards(:clubs_12))
    game.reload
    expect(game_state.attacker).to eq player_two
    expect(game_state.table.cards).to be_empty
    expect(player_two_hand.cards).to be_empty
