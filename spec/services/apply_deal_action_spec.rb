...
RSpec.describe ApplyDealAction do
  ...
  describe "#call" do
    context "player one draws a card from the deck" do
      let(:deal_action) { Action.new(:kind => :deal, :active_card => card, :player => player_one) }
    ...
    context "player two draws a card from the deck" do
      let(:deal_action) { Action.new(:kind => :deal, :active_card => card, :player => player_two) }
    ...
    context "when player one gets the lowest trump card" do
      let(:deal_action) { Action.new(:kind => :deal, :active_card => cards(:hearts_6), :player => player_one) }
    ...
    context "when player two gets the lowest trump card" do
      let(:deal_action) { Action.new(:kind => :deal, :active_card => cards(:hearts_6), :player => player_two) }
    ...
    context "when neither player has a trump card" do
      let(:deal_action) { Action.new(:kind => :deal, :active_card => cards(:diamonds_6), :player => player_one) }
