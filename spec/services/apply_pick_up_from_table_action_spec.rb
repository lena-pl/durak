  ...
  describe "#call" do
    context "when there is at least one card on the table" do
      ...
      context "player one picks up a card from the table" do
        let(:pickup_from_table_action) { Action.new(:kind => :pick_up_from_table, :active_card => card, :player => player_one) }
      ...
      context "player two picks up a card from the table" do
        let(:pickup_from_table_action) { Action.new(:kind => :pick_up_from_table, :active_card => card, :player => player_two) }
